...
const Pong = () => {
	...
	useEffect(() => {
		const intervalId = setInterval(() => {
			...
			if (nextX < 0 || nextX > GAME_WIDTH - BALL_SIZE) {
				setBallDirection((ballDirection) => ({ x: -ballDirection.x, y: ballDirection.y }));
			}
			...
			// Check for scoring
			if (nextX < 0) {
				setScore((score) => ({ player1: score.player1 + 1, player2: score.player2 }));

				setBallPosition({ x: GAME_WIDTH / 2, y: GAME_HEIGHT / 2 });
				setBallDirection({ x: 1, y: 1 });
			}
			if (nextX > GAME_WIDTH - BALL_SIZE) {
				setScore((score) => ({ player1: score.player1, player2: score.player2 + 1 }));
				setBallPosition({ x: GAME_WIDTH / 2, y: GAME_HEIGHT / 2 });
				setBallDirection({ x: -1, y: -1 });
			}
	...
	const winner = score.player1 >= 5 ? 'Player 1' : score.player2 >= 5 ? 'Player 2' : null;

	return (
		<div className="Pong" ref={gameBoardRef}>
			<div className="Paddle Paddle1" style={{ top: paddle1Position, left: 0 }} />
			<div className="Paddle Paddle2" style={{ top: paddle2Position, right: 0 }} />
			<div
				className="Ball"
				style={{
					top: ballPosition.y,
