...
interface IUserBoard {
	...
	isHomeReady: boolean;
	isAwayReady: boolean;
	handleReady: MouseEventHandler<HTMLButtonElement>;
}

export function UserBoard(
	{ home, away, isHomeReady, isAwayReady, handleReady }: IUserBoard
) {
	const { user } = useUser();

	useEffect(() => {
	}, [home, away, isHomeReady, isAwayReady])
	...
	return (
		<div className="user-board d-flex center justify-between px-4">
			...
			{(home.id === user.id || away.id === user.id) && (
				<button className="readyBtn" onClick={handleReady}>Ready</button>
			)}
			<style jsx>{`
				.user-board {
					width: 600px;
